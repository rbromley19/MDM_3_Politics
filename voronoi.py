    ...
    for i in range(len(region)):
        ...
        for j in range(len(region) - 1):
            if comparepoint(region[j], region[j + 1], centroid):
                ...
                print(region[i].evalf())
                print("is greater than")
                print(region[i + 1].evalf())

    # region.sort(key=lambda x: sp.Line(x,centroid).angle_between(xaxis).evalf(), reverse=True)
    for p in region:
        print(sp.Line(centroid, p).angle_between(xaxis).evalf())
...
# print(np.shape(newvertexcoords))
# print(newvertexcoords)
for i in newvertexcoords:
    print(i)
    for j in i:
        print(j)
        vertexcoords[i, j] = sp.Point(j)
n = 5
centrepopulation = 10

i = -1

layers = 4 # increase for better volume accuracy
values = np.linspace(2, -2, n)
def popfunc(x, y, centrepopulation=8):
    return centrepopulation - x * 2 - y * 2
def popfield(centrepopulation=8,N=n):
    X = np.linspace(-2, 2, N)
    Y = np.linspace(-2, 2, N)
...
def tactvoting(tacticalpop,regionvolumes,centroids):
    ...
    print(n)
    values = np.linspace(-2,2,n)
    tacchoices = [[0 for x in range(n)] for y in range(n)]
    print()
    print("TACT")
    #print(tacticalpop)
    for x in tacticalpop:
        ...
        for y in x:
            ...
            for k in range(len(regionvolumes)):
                   #print(centroids)
                   a = regionvolumes[k]/((centroids[k][0]- coordx)**2 +(centroids[k][1]- coordy)**2)**1/2
                   if a>maxratio:
                       maxratio=a
                       regionchoice =k
            #print(tacchoices)
...
for row in tactchoices:
    ...
    for choice in row:
        ...
        print(withtac[choice])
        withtac[choice]+=tacticalpop[k,j]
...
for region in vertexcoords:
    ...
    totalarea += abs(sp.Polygon(*region).area.evalf())
    print("volume/votes of region:" + str(regionvolumes[n]))
    print("Including Tactical voting:"+str(withtac[n]))
    #for point in region:
        #print(point.evalf())
